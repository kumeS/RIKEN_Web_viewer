...
// ここを更新すること
var PER_PIXEL_DEFAULT = 1.0/170.0;
// dzi のデフォルトの画像名
...
{ 
	...
	*/
	...
	var viewer = new OpenSeadragon({ // new OpenSeadragon's viewer
				...
				defaultZoomLevel: 0.0, // image's default zoom level
...
/// アノテーションの座標情報 01
	PinManager.prototype.drawPin1 = function(pos,pinID,labelTxt)
	{			
		...
		$(pin1).addClass('pin ' + id);
		$(pin1).addClass('pin_mark'); //alert(String($(pin).height()));
		$(pin1).attr('data-id',id);
		...
		$(pin1).draggable({
			...
			start:function(e)
			{
				...
				pinDown = true; // pinを押しました
				movedX = $(pin1).offset().left - pin.width / 2; // ピンの座標
				movedY = $(pin1).offset().top;
				downX = e.pageX; // mousedown座標
				downY = e.pageY;
			...
			*/
			...
			$('#pin1').on('mousedown', function()
			{
				...
				if(isPin1)
				{
					...
					$('#pin1 img').attr("src","images/ui_btns_annotation_active01.svg");
					$('#pin').toggleClass('off');
					$('#pin img').attr("src","images/ui_btns_annotation.svg");
				}else
				{
