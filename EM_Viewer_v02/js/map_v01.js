...
// CopyURIのベースのパス  "........" をうめる
var BASE_URL = "http://clst.multimodal.riken.jp/CLST_ViewerData/........./index_v01.html";
...
function SnapShotManager()
	{	
		...
		//this.snapShotBtn = $('#snapshot_shortcut');
...
//$('#snapshot_shortcut').on('click',function(){
//	SnapShotManager.snapShotBtn = $('#snapshot_shortcut');
//		//this.snapShotBtn = $('#snap_btn');
//		SnapShotManager.snapList = [];
//		
//		SnapShotManager.addEvents();
//	});
/// スナップショットここまで
...
//// ここまで
	...
	$("#delete_all_btn").on(
				...
				alert("All Annotation Data was removed");
				});
	...
	{
		//alert(isPin);
		if(measureManager.isEnable || isPin  ) // || isPin1
		{
			viewer.gestureSettingsMouse.clickToZoom = false;
		}
	...
	MeasureManager.prototype.addEvents = function()
	{
		...
		this.toggleStatus = function()
		{
			...
			if(self.isEnable)
			{
				...
				$('#measure_btn img').attr("src","images/ui_btns_measure_active1.png");
				$('#measure_shortcut').attr("src","images/ui_btns_measure_active1.png");
			}else
			{
				$('#measure_btn img').attr("src","images/ui_btns_measure1.png");
				$('#measure_shortcut').attr("src","images/ui_btns_measure1.png");
				self.removeMarks();
	...
	var sectionZoombright = new SectionHeader("zoombright");
			...
			*/
			...
			$('#pin').on('mousedown', function()
			{
				...
				if(isPin)
				{
					...
					$('#pin img').attr("src","images/ui_btns_annotation_active1.png");
					$('#pin_shortcut').attr("src","images/ui_btns_annotation_active1.png");
					$('#pin1').toggleClass('off');
					$('#pin1 img').attr("src","images/ui_btns_annotation.svg");
				}else
				{
					$('#pin img').attr("src","images/ui_btns_annotation.svg");
					$('#pin_shortcut').attr("src","images/ui_btns_annotation.svg");
...
// BRIGHTNESS & CONTRAST
	...
	{
	var	xxx = $("#amount").val();
	var numx = parseInt(xxx);
viewer.setFilterOptions({
    filters: {
        processors:    [ OpenSeadragon.Filters.BRIGHTNESS(numx)]
    }
});
}
	//var	xxx1 = $("#amount1").val();
	//var numx1 = parseInt(xxx1);
//viewer.setFilterOptions({
  //  filters: {
    //    processors:    [ OpenSeadragon.Filters.CONTRAST(numx1)]
    //}
//});
...
// 指定のアノテーション情報を読み込み（１）
	...
	{
		// こんな感じでファイル内をコピーする
		/*var str = '{"annotations":[{"x":0.44440987234042684,"y":0.4798270978723402,"ID":"1502341440913","label":""},{"x":0.8764245787234055,"y":0.3321161872340423,"ID":"1502341441561","label":""},{"x":0.4539396085106395,"y":0.028752919148936018,"ID":"1502341442288","label":""},{"x":0.1569294978723419,"y":0.6894812936170209,"ID":"1502341443088","label":""}],"snapshots":[]}';
		*/
		
		// アノテーション情報を書く
		var str = ''
		
		var res = confirm("Do you replace the annotation of XXX?");
		
		//alert(str)
		if( res == true ) {
		var obj = $.parseJSON(str);		
		//alert(obj)
		//alert(obj.annotations)
...
/*
	...
	{		
	//https://github.com/imageMetadata/OME/blob/master/README.md
	 httpObj = new XMLHttpRequest();
        httpObj.open("get", $('#uri01').val(), true);
...
//開発者アノテーション情報の読み込み
/*
$("#file_importData_A").change(function(e)
	{
        var file = e.target.files[0]
        //var file = new File();
		//alert(file);
